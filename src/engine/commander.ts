...
import { waterEstimate, foodEstimate, daysLabel } from './estimate.js';
...
export interface CommanderReply {
  ...
  avoid: string[];
  ifChange: string[];
  question?: string; // asked only when a critical fact is missing
}
...
function parse(msg: string): Parsed {
  ...
  for (const w of ITEM_WORDS) {
    if (t.includes(' ' + w + ' ') || t.includes(' ' + w + ',')) p.items.push(w);
  }
  return p;
}
...
export function commander(msg: string, state: AppState): CommanderReply {
  const p = parse(msg);
  ...
  if (p.items.length > 0 && /(make|build|improvise|use|do with|with (a|an|my))/.test(msg.toLowerCase())) {
    ...
    const ideas: string[] = [];
    if (p.items.includes('tarp')) ideas.push('Tarp: rain shelter, sun shade, ground barrier, or a rain-collection surface.');
    if (p.items.includes('rope')) ideas.push('Rope: secure a tarp shelter, bundle supplies, or make a clothesline.');
    if (p.items.includes('bucket')) ideas.push('Bucket: store/carry water (if food-safe), collect rain, or line for sanitation.');
    if (p.items.includes('bottle') || p.items.includes('bottles')) ideas.push('Bottles: store safe water and diffuse a flashlight into soft room light.');
    if (p.items.includes('flashlight')) ideas.push('Flashlight: primary light and three-flash signalling.');
    if (p.items.includes('radio')) ideas.push('Radio: official information without the internet.');
    if (ideas.length === 0) ideas.push('Open the IMPROVISE and "Can I use this?" tools for safe, specific uses.');
    return {
