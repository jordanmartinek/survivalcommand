...
import { DecisionNode } from '../model/types.js';
...
export function decisionView(params: Record<string, string>): Node {
  ...
  const byId = new Map<string, DecisionNode>(tree.map((n) => [n.id, n]));
        ...
        h(
          'button',
          {
            ...
            onclick: () => {
              ...
              else if (opt.result) renderResult(opt.result);
            },
          },
          opt.label
        )
  ...
  function renderResult(result: string) {
    ...
    container.appendChild(
      h(
        'div',
        { class: 'priority pri-serious' },
        h('div', { class: 'pri-tag' }, 'Recommended path'),
        h('div', { class: 'pri-action', style: { fontSize: '1.05rem' } }, result)
      )
    );
    container.appendChild(
      h(
        ...
        h('button', { class: 'btn', onclick: () => renderNode(tree[0].id) }, 'Restart'),
        h('button', { class: 'btn primary', onclick: () => navigate('/scenario/' + s!.id) }, 'Full plan')
      )
    );
  }
  ...
  return frag(
    header(),
    backLink('/scenario/' + params.id, s.title),
    h('h1', { class: 'page-title' }, '🌳 ' + s.title + ' — Decisions'),
    h('p', { class: 'page-sub' }, 'Answer each question to reach the recommended action.'),
    container
  );
}
