...
export function waterPerPersonPerDay(state) {
    const base = 3; // drinking + minimal cooking/hygiene, liters
    let mult = 1;
    if (state.household.climate === 'hot')
        mult += 0.6;
    if (state.household.climate === 'cold')
        mult -= 0.1;
...
export function waterEstimate(state) {
...
export function foodEstimate(state) {
    const people = Math.max(1, state.household.people);
...
export function lightingHours(state) {
    return state.resources.lighting;
}
export function powerEstimate(state) {
