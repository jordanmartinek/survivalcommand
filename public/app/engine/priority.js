...
export function computePriority(state) {
    const h = state.household;
    ...
    // ---- Shelter / structural ------------------------------------------
    {
        ...
        if (score > 0) {
            score += vuln;
            if (vuln)
                bd.push({ factor: 'Vulnerable household members', points: vuln });
            items.push({
                key: 'shelter',
                score,
                title: 'Shelter',
                action: c.shelter === 'unsafe'
    ...
    // ---- Temperature ----------------------------------------------------
    {
        ...
        if (score > 0) {
            ...
            items.push({
                ...
                action: 'Move everyone to the safest temperature-controlled space and protect vulnerable people; in heat, hydrate and cool — in cold, stay dry and insulate.',
    ...
    // ---- Water ----------------------------------------------------------
    {
        ...
        const bd = [{ factor: 'Baseline hydration need', points: 2 }];
        ...
        score += vuln * 0.5;
        if (vuln)
            bd.push({ factor: 'Vulnerable members', points: vuln * 0.5 });
        items.push({
            key: 'water',
            score,
            title: 'Water',
            action: c.waterService === 'unsafe'
    ...
    {
        let score = 1;
        const bd = [{ factor: 'Baseline nutrition need', points: 1 }];
    ...
    // ---- Communications -------------------------------------------------
    {
        ...
        if (score > 0) {
            items.push({
                ...
                score,
                title: 'Communication',
                action: 'Conserve device battery, use a radio for official information, and fall back to your family meeting plan.',
    ...
    // ---- Evacuation (derived) ------------------------------------------
    {
        const triggers = [];
        if (c.shelter === 'unsafe')
            triggers.push('unsafe shelter');
        if (c.temperature === 'dangerous')
            triggers.push('uncontrollable temperature');
        if (water.critical && c.waterService !== 'working')
            triggers.push('critically low water');
        if (triggers.length >= 2) {
            items.push({
                key: 'evacuation',
                score: 7 + vuln,
                title: 'Evacuation assessment',
                action: 'Assess whether relocating to a safer location with working infrastructure is now safer than staying — use official information and evacuation orders.',
