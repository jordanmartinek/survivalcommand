...
export function familyView() {
    ...
    return frag(header(), backLink('/prepare', 'Prepare'), h('h1', { class: 'page-title' }, '👪 Family Plan'), h('p', { class: 'page-sub' }, 'Stored locally and offline. This is your shared emergency plan and offline info card.'), infoBox('Write it down so it works even with no phone, no signal, and dead batteries.'), h('div', { class: 'card' }, ...fields), h('button', { class: 'btn primary block', onclick: () => downloadPlan(state.family) }, '⬇ Download offline card'));
}
function downloadPlan(fp) {
    const text = [
...
const STATUS_CYCLE = ['unknown', 'safe', 'help', 'evacuated', 'missing'];
export function communityView() {
    ...
    function renderList() {
        ...
        state.community.forEach((p) => {
            ...
            container.appendChild(h('div', { class: 'list-row' }, h('span', { class: 'lr-icon' }, '👤'), h('div', { class: 'lr-body' }, h('div', { class: 'lr-title' }, p.name || 'Unnamed'), h('div', { class: 'lr-sub' }, p.needs ? 'Needs: ' + p.needs : 'No needs noted')), h('button', { class: 'badge ' + meta.cls, style: { border: 'none', cursor: 'pointer' }, onclick: () => {
    ...
    const needsInput = h('input', { type: 'text', placeholder: 'Needs (optional)' });
    return frag(header(), backLink('/prepare', 'Prepare'), h('h1', { class: 'page-title' }, '🤝 Community'), h('p', { class: 'page-sub' }, 'Tap a status to cycle it. Help vulnerable people where it is reasonably safe to do so.'), container, h('div', { class: 'card' }, h('h3', {}, 'Add a person'), h('label', { class: 'field' }, h('span', { class: 'fl' }, 'Name'), nameInput), h('label', { class: 'field' }, h('span', { class: 'fl' }, 'Needs'), needsInput), h('button', { class: 'btn primary block', onclick: () => {
            if (!nameInput.value.trim())
                return;
            const person = { id: 'p' + Date.now(), name: nameInput.value.trim(), status: 'unknown', needs: needsInput.value.trim() };
