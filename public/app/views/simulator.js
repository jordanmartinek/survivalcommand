...
import { header, backLink, warnBox, infoBox } from '../ui/components.js';
...
export function simulatorView() {
    ...
    function simulate(sc) {
        ...
        let vuln = 'water';
        if (water.daysRemaining !== null && (water.critical || (sc.heat && water.conserve)))
            vuln = 'water';
        else if (food.critical)
            vuln = 'food';
        else if (sc.heat)
            vuln = 'heat exposure';
        else if (sc.power)
        ...
        container.appendChild(h('div', { class: 'card' }, ...rows.map((r) => h('div', { class: 'kv' }, h('span', { class: 'k' }, r.k), h('span', { class: 'v', style: r.risk ? { color: '#f87171' } : {} }, r.v)))));
        container.appendChild(warnBox('Largest vulnerability', ['Your largest vulnerability is ' + vuln + '.']));
        container.appendChild(h('div', { class: 'card' }, h('h3', {}, 'Recommended actions'), h('ol', {}, h('li', {}, vuln === 'water' ? 'Secure additional safe water and reduce non-essential use.' : 'Address ' + vuln + ' as your first priority.'), h('li', {}, 'Establish a backup for your weakest resource.'), h('li', {}, 'Reduce consumption of scarce resources.'), h('li', {}, 'Reassess evacuation options if the location cannot sustain everyone.'))));
    ...
    return frag(header(), backLink('/', 'Home'), h('h1', { class: 'page-title' }, '🔮 What If?'), h('p', { class: 'page-sub' }, 'Simulate a scenario against your current supplies.'), h('div', { class: 'chips' }, ...SCENARIOS.map((sc) => h('button', { class: 'chip', onclick: () => simulate(sc) }, sc.label))), h('div', { class: 'spacer' }), container);
}
// EVACUATE OR SHELTER? decision aid.
export function evacuateView() {
    return frag(header(), backLink('/', 'Home'), h('h1', { class: 'page-title' }, 'Evacuate or Shelter?'), h('p', { class: 'page-sub' }, 'Weigh both options. Never evacuate solely on a prediction — use official orders and current information.'), h('div', { class: 'card' }, h('h3', {}, '🏠 Stay'), h('p', {}, 'Advantages: existing shelter, existing supplies, familiar environment.'), h('p', {}, 'Risks: structural danger, fire, flood, extreme temperature, lack of water, infrastructure failure.')), h('div', { class: 'card' }, h('h3', {}, '🚗 Leave'), h('p', {}, 'Advantages: functioning infrastructure, medical help, food and water, safer environment.'), h('p', {}, 'Risks: traffic, unsafe roads, fuel scarcity, weather, unknown destination.')), warnBox('Decide to leave now if any of these are true', [
